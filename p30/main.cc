    ...
    for (int iW = 0; iW < width; iW++) {
        for (int iH = 0; iH < height; iH++) {
            ...
            for (int t = 0; t < height; t++) {
                ...
                sum.real = sum.real + dftData[width * t + iW].real * cos(angle) + dftData[width * t + iW].imag * sin(angle) *expOp;
                //sum.imag = sum.imag - dftData[width * t + iW].real * sin(angle);
                sum.imag = sum.imag - dftData[width * t + iW].real * sin(angle) * expOp + dftData[width * t + iW].imag * cos(angle);
            }        
